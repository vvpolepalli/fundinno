...
class upload_project_images extends CI_Controller 
 {
  function __construct()
   ...
   function index() {
	...
	//if (in_array($fileParts['extension'],$fileTypes)) {
		...
		$targetFile = rtrim($targetPath,'/').'/images/original/'.$imgpath;
		if(move_uploaded_file($tempFile,$targetFile)){
			
	
			$thumb_path  = rtrim($targetPath,'/').'/images/200x150/'.$imgpath;
			$medium_path = rtrim($targetPath,'/').'/images/medium/'.$imgpath;
			$resize_path = rtrim($targetPath,'/').'/images/img/'.$imgpath;
			...
			$h = $imagedata[1];
		//	if($h >=150 && $w>=200){
			 $this->resizeImageWdthHegt($targetFile,233,140,$thumb_path);
			 $this->resizeImageWdthHegt($targetFile,319,190,$medium_path);
