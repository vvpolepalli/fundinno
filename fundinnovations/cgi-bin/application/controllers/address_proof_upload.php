...
class address_proof_upload extends CI_Controller 
 {
  function __construct()
   ...
   function index() {
	   $targetFolder = 'uploads/user_address_proof'; // Relative to the root
...
if (!empty($_FILES)) {
	...
	$fileParts = pathinfo($_FILES['Filedata']['name']);
		...
		$targetFile = rtrim($targetPath,'/').'/original/'.$filename;
		if(move_uploaded_file($tempFile,$targetFile)){
			$imgpath  = $filename;//uniqid(rand(1,999),false).$this->just_clean($_FILES['Filedata']['name']);
	if (in_array($fileParts['extension'],$fileTypes)) {
			$thumb_path=rtrim($targetPath,'/').'/thumb/'.$imgpath;
			//$medium_path=rtrim($targetPath,'/').'/images/medium/'.$imgpath;
		    $imagedata = @getimagesize($targetFile);
			$w = $imagedata[0];
			
			$h = $imagedata[1];
		//	if($h >=150 && $w>=200){
			 $this->resizeImageWdthHegt($targetFile,330,190,$thumb_path);
			// $this->resizeImageWdthHegt($targetFile,330,190,$medium_path);
		}
			//@unlink($targetFile);
			echo $imgpath;
			//}else
