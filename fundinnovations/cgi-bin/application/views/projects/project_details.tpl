...
<section  class="innerMidWrap">
  ...
  <div class="innerMidContent">
    <div class="disProDetailsLeft">
      ...
      <span class="creterplace">{$project_det.city_name}</span> <span class="creterfield">{$project_det.category_name|ucwords}</span>
      <div class="clear"></div>
...
<!--<a class="addthis_button_linkedin in" style="cursor:pointer"><img src="" alt="in" /></a>--></li>
              <!--<a href="https://twitter.com/share" class="twitter-share-button" data-text="{$project_det.project_title}" data-count="none">Tweet</a>-->
      ...
      <div class="videoBigPane" {if $project_det.vid_sts eq 'no'} onclick="video_stschk('{$project_det.intro_video}')" {/if}>
   <!--[if lt IE 9]> 
{literal} 
<script type="text/javascript">
$(function() {
   ...
   <!-- <track kind="captions" src="captions.vtt" srclang="en" label="English" />-->
    ...
    <div class="disProDetailsRight">
      ...
      <div class="createrDetails">
        ...
        <ul>
          <li class="createrName"><a href="{$baseurl}profile/index/{$project_det.user_id}">{$project_det.profile_name|ucwords}</a></li>
          <li>
      ...
      <div class="createrDetails">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <th width="50%" scope="col"><span class="label1">Days to go</span> <br>
              <span class="derails1">{if $project_det.remaining_days lte 0 || $project_det.remaining_days eq ''}
              {if $project_det.created_date eq '' || $project_det.created_date eq '0000-00-00 00:00:00'}-{else}-{/if}
              {else}
              {if $project_det.remaining_days gt $project_det.fund_duration}
              -
              {else}
              {$project_det.remaining_days}
              {/if}
              {/if}</span></th>
          </tr>
          <tr>
        ...
        <ul class="details2">
          ...
          <div class="label"><span class="left">Project start date</span><span class="right">:</span></div>
            <div class="details">{if $project_det.created_date eq '' || $project_det.created_date eq '0000-00-00 00:00:00'}{else}{$project_det.created_date|date_format}{/if}</div>
            <div class="clear"></div>
        ...
        <div class="blueBtnLi left">
          <input type="button" class="blueBtn" id="btn_fund_project" name="btn_fund_project"  {if $project_det.user_id neq $user_id} 
          {if $project_det.project_status eq 'ongoing' && $project_det.status neq 'rejected'}
          {if $project_det.remaining_days gt 0 && $project_det.remaining_days lte $project_det.fund_duration}
        {if $project_det.backers_cnt lt $project_det.max_sponsors || $project_det.max_sponsors eq 0}
        
          onclick="fund_project_pop({$proj_id})"
          {/if} {/if} {/if}{/if} value="Pre-Order Items">
        </div>
      ...
      <div class="promotePane">
        <div class="promotePaneInner">
         {if $project_det.user_id neq $user_id && $project_det.project_status eq 'ongoing'}
...
addthis:title="{$project_det.project_title}"
        addthis:description="{$share_contant}">
            <div class="custom_images">
              <ul class="socialShare socialShareblck" {if $project_det.promote_link eq ''}  style="display:none;" {/if}>
                <li>
                <a target="_blank" class="fb show_soc" href="https://www.facebook.com/dialog/feed?app_id=316458278483601&link={$project_det.promote_link}&picture={$baseurl}uploads/projects/images/medium/{$project_det.project_image}&name={$project_det.project_title}&caption={$project_det.project_title}&description={$share_contant}&redirect_uri={$baseurl}archive_projects/project_details/{$project_det.id}">share</a>
       ...
       {assign var="back_sts" value=''}
      {if $project_det.project_status eq 'ongoing' && $project_det.status neq 'rejected'}
     {if $project_det.remaining_days gt 0 && $project_det.remaining_days lte $project_det.fund_duration}
       {if $project_det.backers_cnt lt $project_det.max_sponsors || $project_det.max_sponsors eq 0}
      {assign var="back_sts" value='yes'}
     {/if}
      {/if}
       {/if}
      {if $project_det.rewards neq 0}
      {foreach from=$project_det.rewards item=reward }
    
      <div class="pledgeMorePane">
        <h4   {if $project_det.user_id neq $user_id }
        {if $reward.taken_cnt gte $reward.users_limit && $reward.users_limit neq 0}
        {else}
        {if $back_sts eq 'yes'}
         {if $reward.click_sts eq 1}
         onclick="fund_reward_pop({$proj_id},{$reward.id})" style="cursor:pointer;padding-bottom:5px;"
         {/if}
         {/if}
         {/if}
          {/if}  >Pre-Order <span class="WebRupee">Rs. </span>{$reward.pledge_amount}<!-- or more--></h4>
       <div style="padding:0 0 4px;color:#999">{$reward.taken_cnt} Supporters {if $reward.taken_cnt eq $reward.users_limit && $reward.users_limit neq 0}<span class="sold_out">Sold out</span>{/if}</div>
