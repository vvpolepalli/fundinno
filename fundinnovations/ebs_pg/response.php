...
$response['Merchant_Id']= $_REQUEST['Merchant_Id'];
	$response['Amount']= $_REQUEST['Amount'];
	$response['Order_Id']= $_REQUEST['Order_Id'];
	$response['Merchant_Param'] = $_REQUEST['Merchant_Param'];
	$response['Checksum']= $_REQUEST['Checksum'];
	$response['AuthDesc']=$_REQUEST['AuthDesc'];
	...
	// $response = array();
	 ...
	 $parameter=rawurlencode(serialize($response));
		...
		$orderurl= "../archive_projects/processOrder/".$parameter;
